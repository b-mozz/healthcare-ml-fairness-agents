...
from sklearn.model_selection import train_test_split # creates train/test splits (trains on 70%, test on 30%)
from sklearn.ensemble import RandomForestClassifier # a proxy model to generate predictions, n_estimator=10
from sklearn.preprocessing import LabelEncoder # converts categoical text to numeric labels
from sklearn.impute import SimpleImputer # to replace missing values with NaN
...
def run_generic_audit(file_path, target_column):
    ...
    # arges: X_.. --> dataFrame. y = target_column, test_size=0.3--> 30% for testing
...
if __name__ == "__main__":
    if len(sys.argv) > 2: # check if user provided atleast 3 command line arguments
