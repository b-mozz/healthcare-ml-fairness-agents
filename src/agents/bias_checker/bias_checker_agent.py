...
from fairlearn.metrics import (
    MetricFrame,                        # Framework to calculate metrics across demographic groups
    demographic_parity_difference,      # Measures absolute difference in selection rates between groups (range: -1 to 1, ideal: 0)
    demographic_parity_ratio,           # Measures ratio of selection rates between groups (range: 0 to infinity, ideal: 1)
    equalized_odds_difference,          # Measures difference in TPR and FPR across groups (range: 0 to 1, ideal: 0)
    selection_rate,                     # Proportion of positive predictions (y_pred=1) for each group
    true_positive_rate,                 # TPR: Of actual positives, how many were predicted positive (sensitivity/recall)
    false_positive_rate,                # FPR: Of actual negatives, how many were incorrectly predicted positive
    count                               # Count of samples in each demographic group
)
...
class BiasReport:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        def safe_round(val):
            ...
            if isinstance(val, float): # checks if value is a float obj/type
                return round(val, 4)
            elif isinstance(val, dict): # checks if val is a dictionary
                return {k: safe_round(v) for k, v in val.items()} # safe_round rounds safely and it is recursive
...
class BiasChecker:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.config = {
            ...
            "demographic_parity_ratio_min": 0.8, # EEOC --> 80% rule
            "demographic_parity_ratio_max": 1.25, # EEOC --> 1/0.8 
    ...
    def check(
        self,
        y_true: Union[np.ndarray, pd.Series, List], # union from python's typing module, meaning this parameter can be either of the following
        ...
        for attr_name, attr_values in sensitive_features.items():
            ...
            # append adds whole thing as one item
    ...
    def _validate_inputs(
        ...
        # this needs modification as many columns do not have binary classifications
