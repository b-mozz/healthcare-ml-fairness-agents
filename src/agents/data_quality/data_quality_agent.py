...
class ValidationReport:
    ...
    summary: Dict[str, Any]               # Quick stats {rows: 1000, columns: 17, ...}; any is imported from typing library, which means any type is accepted.
    ...
    # we are using field() function cause ml_methods_used has a default value, so by initializing it we are making it optional.
...
class DataQualityValidationAgent:
    ...
    def _check_schema(self, df: pd.DataFrame) -> None:
        ...
        # does not detect white space or '' --> empty string as unnamed. MODIFICATION needed
        ...
        duplicate_mask = df.duplicated() # [true, false, true,...]
        ...
        for id_col in id_columns:
            if id_col in df.columns:
                ...
                if dup_count > 0:
                    ...
                    dup_values = df.loc[dup_mask, id_col].tolist() # selects patient id value where dup_musk = 1
    ...
    def _check_statistical_outliers(self, df: pd.DataFrame) -> None:
        ...
        for col in df.columns:
            ...
            if not pd.api.types.is_numeric_dtype(df[col]):
                continue # takes the code back to the for loop
    ...
    def _check_anomalies_ml(self, df: pd.DataFrame) -> None:
            ...
            try:
                ...
                # ensemble methods combine multiple models to make better prediction, i think this is the best way to define it
                # so isolation FOREST (literally in the name) basically creates multiple decision trees
            ...
            iso_forest = IsolationForest(
                ...
                random_state=42, # for reproducibility. also to make the code faster, Hichhiker's guide to galaxy
                n_jobs=-1 # use all CPU cores
            ) # n_estimators not specified, so uses default 100 trees.
            iso_predictions = iso_forest.fit_predict(numeric_data) # 1 normal, -1 anamoly
            iso_scores = iso_forest.decision_function(numeric_data) # lower = more anamoly. shallow trees. a person standing alone is easier to spot
            self.ml_methods_used.append("Isolation Forest")
            
            # Method 2: Local Outlier Factor
            # using both catches more types of anamoly
            # O(N^2) --> not efficient. choose a different algorithm. UPDATE
            lof = LocalOutlierFactor(
                n_neighbors=20, # 20 nearest neighbors, norm (IBM blog)
                contamination=contamination,
                novelty=False #Training mode is false, we are not going to use this model on clean data
            )
